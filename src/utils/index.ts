  ...
  Object.keys(source).forEach(property => {
    ...
    if (typeof sourceProperty === 'object') {
      target[property] = objectMerge(
        target[property] as Record<string, unknown>,
        sourceProperty as Record<string, unknown> | Record<string, unknown>[]
      )
    } else {
...
export function deepClone<
  T extends Record<string, unknown> | Record<string, unknown>[] = any
>(source: T): T {
  ...
  Object.keys(source).forEach(keys => {
    const item = Array.isArray(source)
      ? source[Number(keys)]
      : (source as Record<string, unknown>)[keys]

    if (item && typeof item === 'object') {
      ;(targetObj as Record<string, unknown>)[keys] = deepClone(item)
    } else {
      ;(targetObj as Record<string, unknown>)[keys] = item
    }
  })
